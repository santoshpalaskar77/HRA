...
parser.add_argument("--input_len",        default=4, type=int, help="input chunk length")
parser.add_argument("--output_len",        default=4, type=int, help="output chunk length")
...
df = pd.read_csv(ARGS.data_path)
freq = 'D'
df.rename(columns = {'Unnamed: 0':'date','Total': 'total'}, inplace = True)
...
'''Highest level is total level we are considering levels after the total, 
    we need *level* in latter part to do the aggregation'''
level1 = np.array(df1.columns[1:7])
level1_r = list(range(1,7))
level2 = np.array(df1.columns[7:25])
level2_r = list(range(7,25))
level3 = np.array(df1.columns[25:49])
level3_r = list(range(25,49))            
level4 = np.array(df1.columns[49:len(df.columns)])
level4_r = list(range(49,len(df.columns)))
level0 = [0]
levels = [level0,level1_r,level2_r,level3_r,level4_r]
bottom_level =  level4_r  #bottom level 
...
level2_h =  {k: [v for v in level3  if v.startswith(k)] for k in level2}
level3_h =  {k: [v for v in level4  if v.startswith(k)] for k in level3}

hierarchy ={**total,**level1_h, **level2_h,**level3_h}

aac = list(hierarchy.keys())   # aggregated series 
aab= list(hierarchy.values())  # child nodes of each aggregated series 


''' Defining aggregation structure that will be needed while getting the aggregated 
...
# %%
scaler_en = Scaler()
series_en_transformed = scaler_en.fit_transform(series_en) #transform the series 
